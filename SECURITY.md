...
- **Backups carry the key by default.** `iai_mcp.backup.backup()` includes `.crypto.key` (and any pre-rotation generation) so an archive restores standalone — which makes the archive exactly as sensitive as the key. It is created 0600 through a temp descriptor and atomic rename (on Windows the ACL is restricted after the rename, the same order as the key writer), and the call logs a `WARNING` naming the key files inside. Treat it like the key, not like a database dump. `backup(include_key=False)` leaves the key files out; such an archive can only be restored where the matching key already exists, and `restore()` refuses to pair it with a pre-existing key unless the caller passes `use_existing_key=True` (the restore is then proven by decrypting the records once). Note that an archive without key files is not ciphertext-only: side stores such as `.memory-bank` hold plaintext snippets and travel in the archive either way.
